...
class BERTLSTMMulattenLogRegCRF(nn.Module):
    ...
    def __init__(self, freeze_bert, tokenizer, device, bidirectional):
        ...
        if bidirectional == True:
            ...
            self.self_attention2 = nn.MultiheadAttention(self.hidden_dim * 2, 8, bias=True) # attention mechanism from PyTorch
        else:
            # self.attn = MultiAttention(self.hidden_dim, device)
            self.self_attention = nn.MultiheadAttention(self.hidden_dim, 1, bias=True)
            self.self_attention2 = nn.MultiheadAttention(self.hidden_dim, 8, bias=True) 
    ...
    # def forward(self, args, input_ids=None, attention_mask=None, labels=None):
        ...
        attention_applied, attention_weights = self.self_attention2( lstm_output, lstm_output, lstm_output, key_padding_mask=None, need_weights=True, attn_mask=None )
