...
class BERTLogReg(nn.Module):
    ...
    def __init__(self, freeze_bert, tokenizer, class_weights_coarse, class_weights_fine):
        ...
        self.hidden2tag = nn.Linear(768, clf_P_num_labels)

        # log reg (for fine labels)
        self.hidden2tag_fine = nn.Linear(768, clf_P_fine_num_labels)
    ...
    def forward(self, args, input_ids=None, attention_mask=None, P_labels=None, P_f_labels=None):
        ...
        # mask the unimportant token labels before log_reg
        mask = (
        ...
        # mask unimportant token vectors
        ...
        # linear for fine
        probablity_fine = F.relu ( self.hidden2tag_fine( summed_last_4_layers ) )
        ...
        loss_coarse = self.loss_fct(probablity.view(-1, clf_P_num_labels), P_labels.view(-1))
        # print('Coarse-grained loss: ', loss_coarse)

        # calculate fine loss
        loss_fine = self.loss_fct_fine(probablity_fine.view(-1, clf_P_fine_num_labels), P_f_labels.view(-1))
