...
class BERTLSTMLogReg(nn.Module):
    ...
    def __init__(self, freeze_bert, tokenizer, device, bidirectional, class_weights_coarse, class_weights_fine):
        ...
        self.loss_fct = nn.CrossEntropyLoss(weight=class_weights_coarse)
        # loss calculation (for coarse labels)
        self.loss_fct_fine = nn.CrossEntropyLoss(weight=class_weights_fine)

    def forward(self, input_ids=None, attention_mask=None, P_labels=None, P_f_labels=None):
        ...
        # print('Before shortening: ', P_labels.shape, ' and : ', P_f_labels.shape)
        P_labels = P_labels[:, :lstm_output.shape[1]]
        P_f_labels = P_f_labels[:, :lstm_output.shape[1]]
        # print('After shortening: ', P_labels.shape, ' and : ', P_f_labels.shape)
        ...
        # print('After making: ', P_labels.shape, ' and : ', P_f_labels.shape)
        ...
        loss_coarse = self.loss_fct(probablity.view(-1, clf_P_num_labels), P_labels.flatten())
        ...
        return loss_coarse, logits.to('cpu').numpy(), P_labels, loss_fine, logits_fine.to('cpu').numpy(), P_f_labels, mask