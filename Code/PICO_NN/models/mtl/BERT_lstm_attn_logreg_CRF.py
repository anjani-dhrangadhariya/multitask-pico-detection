...
class BERTLSTMattenLogRegCRF(nn.Module):
    ...
    def __init__(self, freeze_bert, tokenizer, device, bidirectional, class_weight_c = None, class_weight_f = None):
        ...
        if bidirectional == True:
            ...
            self.self_attention2 = nn.MultiheadAttention(self.hidden_dim * 2, 2, bias=True) # 2-head attention mechanism from PyTorch

        else:
            #self.attn = MultiAttention(self.hidden_dim, device)
            self.self_attention = nn.MultiheadAttention(self.hidden_dim, 1, bias=True) 
            self.self_attention2 = nn.MultiheadAttention(self.hidden_dim * 2, 2, bias=True) # attention mechanism from PyTorch
    ...
    def forward(self, args, input_ids=None, attention_mask=None, P_labels=None, P_f_labels=None):
        ...
        # Apply Bahdanau attention here
        #b_attention_applied, b_attention_weights = self.attention( P_f_labels.float(), lstm_output, lstm_output, mask = attention_mask_)
        ...
        #max_probs = torch.max(probablity_fine, dim=2)         
        #logits_fine = max_probs.indices.flatten()
