...
from models.mtl.BERT_logreg_MTL import BERTLogReg as BERTLogRegMTL
from models.mtl.BERT_lstm_logreg import BERTLSTMLogReg as BERTLSTMLogRegMTL
from models.BERT_logreg_CRF import BERTLogRegCRF
from models.mtl.BERT_lstm_logreg_CRF import BERTLSTMLogRegCRF as BERTLSTMLogRegCRFMTL
from models.mtl.BERT_lstm_attn_logreg_CRF import BERTLSTMattenLogRegCRF as BERTLSTMattenLogRegCRFMTL
from models.mtl.BERT_lstm_Mattn_logreg_CRF import BERTLSTMMulattenLogRegCRF as BERTLSTMMulattenLogRegCRFMTL
from models.mtl.BERT_lstm_Mattn_logreg import BERTLSTMMulattenLogRegCRF as BERTLSTMMulattenLogRegMTL
from models.mtl.BERT_lstm_Mattn_logregC import BERTLSTMMulattenLogRegCRF as BERTLSTMMulattenLogRegCMTL
from models.bioBERT_lstm_logreg_CRF import BioBERTLSTMLogRegCRF
from models.mtl.GPT2_lstm_logreg_CRF import GPT2LSTMLogRegCRF as GPT2LSTMLogRegCRFMTL
from models.mtl.GPT2_lstm_attn_logreg_CRF import GPT2LSTMattenLogRegCRF as GPT2LSTMattenLogRegCRFMTL
    ...
    with torch.no_grad() :
        ...
        for e_input_ids_, e_input_mask, e_labels, e_f_input_ids, e_f_input_mask, e_f_labels in val_loader:
            ...
            for i in range(0, e_labels.shape[0]):
                ...
                selected_preds = torch.masked_select(e_output[i, ].cuda(), mask[i, ])
                selected_labs = torch.masked_select(e_labels[i, ].cuda(), mask[i, ])

                e_cr = classification_report(y_pred=selected_preds.to("cpu").numpy(), y_true=selected_labs.to("cpu").numpy(), labels=list(range(clf_P_num_labels)) , output_dict=True)
        ...
        avg_val_loss = mean_loss / len(val_loader)        
        writer.add_scalar('loss-validation', avg_val_loss, epoch_number)
        avg_val_loss_coarse = total_val_loss_coarse / len(val_loader)     
        writer.add_scalar('loss-validation-coarse', avg_val_loss_coarse, epoch_number)
        avg_val_loss_fine = total_val_loss_fine / len(val_loader)     
        ...
        val_cr = classification_report(y_pred=all_pred_flat_, y_true=all_GT_flat_, labels=list(range(clf_P_num_labels)), output_dict=True)
        print(val_cr)

        all_pred_flat_fine = np.asarray(all_predictions_fine).flatten()
        all_GT_flat_fine = np.asarray(all_GT_fine).flatten()
        val_cr_fine = classification_report(y_pred=all_pred_flat_fine, y_true=all_GT_flat_fine, labels=list(range(clf_P_fine_num_labels)), output_dict=True)
        print(val_cr_fine)
...
def train(defModel, tokenizer, opti, schedulr, train_loader, dev_loader, val_loader, class_weights_coarse, class_weights_fine, args, device, exp_args):

    best_meanf1 = 0.0
    best_meanf1_fine = 0.0
    for epoch_i in range(0, args.max_eps):
        ...
        for step, batch in enumerate(train_loader):
            ...
            # Get BERT vectors from the defined model
            b_loss_coarse, b_output, b_labels, b_loss_fine, b_output_fine, b_f_labels, b_mask, cumulative_loss = defModel(args, b_input_ids, attention_mask=b_input_mask, P_labels=b_labels, P_f_labels=b_f_labels)
            ...
            if step % args.print_every == 0:
                cr = classification_report(y_pred= train_epoch_logits, y_true=train_epochs_labels, labels= list(range(clf_P_num_labels)), output_dict=True)
                print("Iteration {} of epoch {} complete. Loss : {:.4f} F1 mean : {:.8f}".format(step, epoch_i, abs ( b_loss_coarse.item() ), cr['1']['f1-score']))
        ...
        writer.add_scalar('loss-train', avg_train_loss, epoch_i) # # write training loss score to logs

        avg_train_loss_coarse = total_train_loss_coarse / len(train_dataloader)
        writer.add_scalar('loss-train-coarse', avg_train_loss_coarse, epoch_i)

        avg_train_loss_fine = total_train_loss_fine / len(train_dataloader)
        writer.add_scalar('loss-train-fine', avg_train_loss_fine, epoch_i)


        # get classification matrix for the epoch training
        train_cr = classification_report(y_pred= train_epoch_logits, y_true=train_epochs_labels, labels= list(range(clf_P_num_labels)), output_dict=True) 
        if experiment_type == 'NER':
            train_meanF1 = ( train_cr['1']['f1-score'] + train_cr['2']['f1-score'] ) / 2
        elif experiment_type == 'seq_lab':
            train_meanF1 = train_cr['1']['f1-score']
        elif experiment_type == 'bioes':
            train_meanF1 = ( train_cr['1']['f1-score'] + train_cr['2']['f1-score'] + train_cr['3']['f1-score'] + train_cr['4']['f1-score'] ) / 4
        writer.add_scalar('f1-train', train_meanF1, epoch_i) # # write training F1 score to logs
        ...
        print('--------------------------------------------------------------------------------')
        # Perform validation over the validation set
        val_loss, val_cr, val_cr_fine, all_pred_flat_, all_GT_flat_, all_pred_flat_fine, all_GT_flat_fine = evaluate_i(defModel, tokenizer, opti, schedulr, dev_loader, args, device, epoch_i)
        if experiment_type == 'NER':
            val_meanF1 = ( val_cr['1']['f1-score'] + val_cr['2']['f1-score'] ) / 2
        elif experiment_type == 'seq_lab':
            val_meanF1 = val_cr['1']['f1-score']
        elif experiment_type == 'bioes':
            val_meanF1 = ( val_cr['1']['f1-score'] + val_cr['2']['f1-score'] + val_cr['3']['f1-score'] + val_cr['4']['f1-score'] ) / 4
        writer.add_scalar('f1-validation', val_meanF1, epoch_i) # write validation F1 score to logs   
        ...
        print("Epoch {} complete! Validation Loss : {}, Validation F1 (coarse) : {:.8f}".format(epoch_i, abs(val_loss), val_meanF1))
        print("Epoch {} complete! Validation Loss : {}, Validation F1 (fine) : {:.8f}".format(epoch_i, abs(val_loss), val_meanF1_fine))

        if val_meanF1_fine > best_meanf1_fine:

            print("Best validation mean F1 improved from {} to {} ...".format(best_meanf1_fine, val_meanF1_fine ))
            
            # save the best model here
            epoch_ij = epoch_i + 0
            model_name_here = '_mtl_' + set_entity +'/' + experiment_type + '/' + str(args.fold) + '/' + args.model_name + '_epoch_' + str(epoch_ij) + '_best_model.pt'
            ...
            best_meanf1_fine = val_meanF1_fine
...
if __name__ == "__main__":
    ...
    print('Coarse weights: ', class_weights_coarse)
        ...
        if fold == 0:
            ...
            for model_name in model_names:
                ...
                if model_name == 'bert_linear':
                    model = BERTLogRegMTL(args.freeze_bert, tokenizer, class_weights_coarse, class_weights_fine)
                    ...
                    if 'bi' in model_name:
                        ...
                        bidirec = False
                    model = BERTLSTMLogRegMTL(args.freeze_bert, tokenizer, device, bidirec, class_weights_coarse, class_weights_fine)
                    ...
                    if 'bi' in model_name:
                        ...
                        bidirec = False
                    model = BERTLSTMLogRegCRFMTL(args.freeze_bert, tokenizer, device, bidirec)

                elif model_name in ['bert_lstm_attn_crf', 'bert_bilstm_attn_crf', 'bert_bilstm_attn_fine_crf', 'bert_bilstm_attn_coarse_crf']:
                    if 'bi' in model_name:
                        print('The model is bidirectional...')
                        bidirec = True
                    else:
                        bidirec = False
                    model = BERTLSTMattenLogRegCRFMTL(args.freeze_bert, tokenizer, device, bidirec, class_weight_c = class_weights_coarse, class_weight_f = class_weights_fine)

                elif model_name in ['bert_bilstm_mattn_crf']:
                    if 'bi' in model_name:
                        bidirec = True
                    else:
                        bidirec = False
                    model =  BERTLSTMMulattenLogRegCRFMTL(args.freeze_bert, tokenizer, device, bidirec, class_weight_c = class_weights_coarse, class_weight_f = class_weights_fine)            

                elif model_name in ['bert_bilstm_mattn_logreg']:
                    if 'bi' in model_name:
                        bidirec = True
                    else:
                        bidirec = False
                    model = BERTLSTMMulattenLogRegMTL(args.freeze_bert, tokenizer, device, bidirec, class_weight_c = class_weights_coarse, class_weight_f = class_weights_fine)
                    ...
                    if 'bi' in model_name:
                        ...
                        bidirec = False
                    model = BERTLSTMMulattenLogRegCMTL(args.freeze_bert, tokenizer, device, bidirec, class_weight_c = class_weights_coarse, class_weight_f = class_weights_fine)
                    ...
                    if 'bi' in model_name:
                        ...
                        bidirec = False
                    model = GPT2LSTMLogRegCRFMTL(args.freeze_bert, tokenizer, device, bidirec)
                    ...
                    if 'bi' in model_name:
                        ...
                        bidirec = False
                    model = GPT2LSTMattenLogRegCRFMTL(args.freeze_bert, tokenizer, device, bidirec)
                ...
                print("Created the optimizer and scheduler objects in {} seconds".format(time.time() - st))

                print('##################################################################################')
                print('Begin training...')
                print('##################################################################################')
                train(model, tokenizer, optimizer, scheduler, train_dataloader, development_dataloader, validation_dataloader, class_weights_coarse, class_weights_fine, args, device, exp_args)
                ...
                with open(EBM_file, 'a+') as f:
                    f.write(model_name)
                    f.write('\n')
                test_loss, test_cr, test_cr_fine, all_pred_flat_, all_GT_flat_, all_pred_flat_fine, all_GT_flat_fine = evaluate_i(test_model, tokenizer, optimizer, scheduler, validation_dataloader, args, device, filewrite=EBM_file)
                if experiment_type == 'NER':
                    test_meanF1 = ( test_cr['1']['f1-score'] + test_cr['2']['f1-score'] ) / 2
                elif experiment_type == 'seq_lab':
                    test_meanF1 = test_cr['1']['f1-score']
                elif experiment_type == 'bioes':
                    test_meanF1 = ( test_cr['1']['f1-score'] + test_cr['2']['f1-score'] + test_cr['3']['f1-score'] + test_cr['4']['f1-score'] ) / 4
                print('f1 score (coarse-grained) on test set (EBM-NLP) for the best model: ', test_meanF1)
                ...
                inhouse_file = 'inhouse.txt'
                with open(inhouse_file, 'a+') as f:
                    f.write(model_name)
                    f.write('\n')
                print('Applying the best model on test set (in-house corpus)...')
                test_loss, test_cr, test_cr_fine, all_pred_flat_, all_GT_flat_, all_pred_flat_fine, all_GT_flat_fine = evaluate_i(test_model, tokenizer, optimizer, scheduler, validation2_dataloader, args, device, filewrite=inhouse_file)
                if experiment_type == 'NER':
                    test_meanF1 = ( test_cr['1']['f1-score'] + test_cr['2']['f1-score'] ) / 2
                elif experiment_type == 'seq_lab':
                    test_meanF1 = test_cr['1']['f1-score']
                    ...
                    test_meanF1 = ( test_cr['1']['f1-score'] + test_cr['2']['f1-score'] + test_cr['3']['f1-score'] + test_cr['4']['f1-score'] ) / 4
                print('f1 score on test set (in-house corpus) for the best model: ', test_meanF1)
